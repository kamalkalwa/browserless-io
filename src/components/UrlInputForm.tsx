...
interface UrlInputFormProps {
    isLoading: boolean;
    onSubmit: (url: string) => Promise<void>; // Function to call on submit
}
...
export default function UrlInputForm({ isLoading, onSubmit }: UrlInputFormProps) {
    ...
    const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
        ...
        if (!isLoading && url) {
            onSubmit(url); // Call the handler passed from the parent
        }
    };

    return (
        // Removed outer div, styling handled by parent column
        <div className="p-6 sm:p-8 space-y-6 bg-white rounded-lg shadow-md h-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                    ...
                    // Add transition and hover:scale
                    className="w-full flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 hover:scale-105" // Added transition-all and hover:scale-105
                >
                    {isLoading ? (
                        <>
                            {/* Spinner SVG */}
